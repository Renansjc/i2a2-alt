...
export const useSupabase = () => {
  ...
  const getInvoices = async (
    page: number = 0,
    pageSize: number = 50,
    sortColumn: string = "data_hora_emissao",
  ...
  const getDashboardStats = async () => {
    ...
    const valorTotal =
      valorData?.reduce((sum, nf) => sum + (nf.valor_total_nota || 0), 0) || 0;
  ...
  const getOperationTypeData = async () => {
    ...
    const entrada =
      data?.filter((nf: any) => nf.tipo_operacao === "0").length || 0;
    const saida =
      data?.filter((nf: any) => nf.tipo_operacao === "1").length || 0;
  ...
  const getInvoiceDetails = async (id: string | number) => {
    const { data, error } = await supabase
      .from("notas_fiscais")
      .select(
        `
        *,
        emitente:empresas!notas_fiscais_emitente_id_fkey(*),
        destinatario:empresas!notas_fiscais_destinatario_id_fkey(*)
      `
      )
      .eq("id", id)
      .single();
  ...
  const getInvoiceItems = async (notaFiscalId: string | number) => {
    const { data, error } = await supabase
      .from("nf_itens")
      .select("*")
